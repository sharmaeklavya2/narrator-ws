...
import { getFileFromList, fetchRawArticleFromFile, fetchRawArticleFromId, getArticleEntryFromQString, fetchRawArticleFromUrl } from "./fetchers.js";
...
function deployMenuSwitcher() {
    ...
    document.getElementById('button-library').addEventListener('click', () => menuSwitcher.show('library-menu'));
    document.getElementById('button-edit').addEventListener('click', () => menuSwitcher.show('edit-menu'));
    document.getElementById('library-back').addEventListener('click', () => menuSwitcher.show('open-menu'));
...
function loadFromLocation(location) {
    ...
    if (ae !== undefined) {
        fetchRawArticleFromUrl(ae.path, true)
            .then((rawArticle) => { rawArticle.id = ae.id; return rawArticle; })
            .then(parseArticle).then(loadArticle).catch(logError);
    }
}
...
function updateLocationWithArticleId(id) {
...
function loadArticle(articleInfo) {
    ...
    const voiceInterPauseElem = document.getElementById('voice-inter-pause');
...
function setupMenuListeners() {
    ...
    setupSlider('voice-inter-pause', 'voice-inter-pause-number', (x) => {
        if (globals.settings !== undefined) {
