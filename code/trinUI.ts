...
export function buildScaffolding(elem: HTMLElement): number {
    let count = 0;
    if(!(elem.classList.contains('trin-scaff-elem') || elem.classList.contains('no-trin-scaff'))) {
        const children = Array.from(elem.childNodes);
        for(const child of children) {
            if(child instanceof Text) {
                const wns: [string, string | undefined][] = [];  // words n' scripts
...
export function trinElem(elem: HTMLElement, targetScript?: string, onHoverOnly: boolean = false): void {
...
export function trinAll(targetScript?: string, onHoverOnly: boolean = false) {
    const trinScaffElems = document.getElementsByClassName('trin-scaff-elem');
    for(const elem of trinScaffElems) {
        trinElem(elem as HTMLElement, targetScript, onHoverOnly);
    }
}
